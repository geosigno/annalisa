      ...
      google
        ...
        .request((err, res, body) => {
          if (err) {
            ...
            axios.get('https://www.googleapis.com/oauth2/v2/userinfo', { headers })
              .then((response) => {
                if (response.data.picture) {

                  const contentTypeHeader = { 'content-type': 'image/jpeg' };
                  const authorizationHeader = {
                    Authorization: 'Bearer ya29.a0AfH6SMCOa5fmAE8bA6CmgOUjFi8GW-Wh9F4agPLBhI_WR18V3IaimdSZdbYyhpBbLwB4kLvrJy0QTcupF8jVwoQPi_G0063Qzemjm7xvmMTkuBIrSGm6rXxGWjqNn5lesTeTGMMMiO41bmyVfaV495fu6zErTVI_1i0zyw'
                  }
                  const headers = { ...contentTypeHeader, ...authorizationHeader };
                  axios.get('https://lh3.googleusercontent.com/a-/AOh14Gg66Hxid8VsJlMbGNCM1U2pY3Yb5r8mys3xwcn0', headers).then((response) => {
                    console.log('response.data',response.data)
                    console.log('sizeof', sizeof(response.data))
                  ...
                  // // return new Promise((resolve) => {
                    axios.post('http://localhost:1337/upload', formData, { headers }).then((response) => {
                      ...
                      // resolve(uploadID);
                      callback(null, {
                        username: response.data.given_name || body.email.split('@')[0],
                        email: response.data.email,
                        avatar: uploadID || null
                      })
                    })
                    .catch((error) => {
                      console.log(error);
